...
class Radtrans(_read_opacities.ReadOpacities):
    ...
    def __init__(
        ...
        self.transmission = np.array(np.zeros((self.g_len, self.freq_len, len(line_species), pressures.shape[0])), dtype='d', order='F')
        self.final_transm = np.array(np.zeros((self.freq_len, pressures.shape[0])), dtype='d', order='F')
        ...
        # enable transmission to be calculated using a physics-informed neural network (PINN)
        ...
        if self.enable_pinn:
            ...
            pinn_directory = os.path.join(self.path_input_data, 'pinn')
            self.batch_size = int(2**22)

            self.model = torch.load(os.path.join(pinn_directory, 'model.pkl'), map_location=self.dev)

            self.mean_std_alpha = torch.cat([torch.linspace(-18.0, -6.0, 100, dtype=torch.float32).unsqueeze(1), torch.linspace(4.5, 4.5, 100, dtype=torch.float32).unsqueeze(1)], dim=1).to(self.dev)
        
        # enable transmission with Rayleigh scattering to be calculated
        # using a physics-informed neural network (PINN)
        self.enable_rayleigh = enable_rayleigh
        if self.enable_rayleigh:
            if not self.enable_pinn:
                if torch.cuda.is_available():
                    self.dev = torch.device('cuda')
                else:
                    self.dev = torch.device("cpu")
            
            rayleigh_pinn_directory = os.path.join(self.path_input_data, 'rayleigh_pinn')
            
            self.rayleigh_model = torch.load(os.path.join(rayleigh_pinn_directory, 'model.pkl'), map_location=self.dev)
            
            self.rayleigh_on = 1
            self.incident_angle = 0.0


    def r_isotherm(self, a, r_P0):
        P0 = 0.01
        pressures = torch.logspace(-6, 2, 100).to(self.dev)
        return r_P0 / (1 + ((a/r_P0)*torch.log(pressures/P0)))        
    
    
    def normalize_inputs(self, alphas, a, R_pl, msa=None):
        if msa == None:
            msa=self.mean_std_alpha

        radii = self.r_isotherm(a, R_pl)
        
        # r0 = radii[0].clone().unsqueeze(0)
        # r99 = radii[-1].clone().unsqueeze(0)

        # mask = alphas < 1e-30
        # alphas[mask] = 1e-30
        # print(alphas.get_device(), msa.get_device())
        # alphas = alphas * torch.sqrt(r0**2 - r99**2)
        alphas = (torch.log10(alphas) - msa[:,0][None,:]) / msa[:,1][None,:]
        # alphas[:,0] = alphas[:,1]
        mask = alphas < -1.5
        alphas[mask] = -1.5
        ...
        R_pl = ((R_pl / nc.r_jup_mean) - 1.05) / 0.95
        a = (torch.log10(a) - 7.1) / 0.35

        inputs = torch.cat([R_pl.reshape(1,1).repeat(n_alphas,1), a.reshape(1,1).repeat(n_alphas,1), alphas], dim=1)
    ...
    def mix_opa_tot(self, abundances, mmw, gravity,
        ...
        if len(self.rayleigh_species) != 0:
            self.scat = True
            self.opa_rayleigh_scat = self.continuum_opa_scat.copy()
            self.add_rayleigh(abundances)
            self.opa_rayleigh_scat = self.continuum_opa_scat - self.opa_rayleigh_scat
        # Add gray cloud deck
    ...
    def py_calc_transm_spec(self,
        ...
        if self.enable_rayleigh:
            continuum_opa_scat -= self.opa_rayleigh_scat.reshape(1, self.freq_len, 1, struc_len)
            # print(self.opa_rayleigh_scat.shape)
            norm_rayScat = torch.tensor((np.log10(self.opa_rayleigh_scat[:,-1] * rho[0,:,0,-1]) + 8.7)/3.0).reshape(self.freq_len, 1).to(self.dev)
            norm_rayScat[norm_rayScat<-2.0] = -2.0
            
            a = torch.tensor((nc.kB * self.temp[0]) / (gravity*mmw[0]*nc.amu)).to(self.dev)
            inputs = self.normalize_inputs(torch.ones(self.freq_len,100).to(self.dev), a, torch.tensor(R_pl).to(self.dev), msa=torch.cat((torch.zeros(100,1), torch.ones(100,1)), dim=-1).to(self.dev))[:,:2]
            x = torch.ones(inputs.shape[0]).unsqueeze(1).detach().to(self.dev)
            mu = torch.zeros(inputs.shape[0]).unsqueeze(1).detach().to(self.dev)
            y = torch.tensor(2 * ((radius - radius[-1]) / (radius[0]-radius[-1])) - 1).unsqueeze(1).detach().to(self.dev)
            theta_max = torch.ones(inputs.shape[0]).unsqueeze(1).detach().to(self.dev) * self.incident_angle

            inputs = torch.cat((x.repeat(100,1), y.repeat_interleave(x.shape[0], dim=0), mu.repeat(100,1), inputs.repeat(100,1), norm_rayScat.repeat(100,1), theta_max.repeat(100,1)), dim=-1).float()
            
            t_graze_rayleigh = self.rayleigh_model(inputs).detach().cpu().numpy()
            if self.rayleigh_on:
                t_graze_rayleigh = t_graze_rayleigh[:,0] + t_graze_rayleigh[:,1]
            else:
                t_graze_rayleigh = t_graze_rayleigh[:,0]
            t_graze_rayleigh = t_graze_rayleigh.reshape(100,-1).swapaxes(0,1).reshape(1, self.freq_len, 1, struc_len)
            
            t_graze_rayleigh[t_graze_rayleigh > 1.0] = 1.0
            t_graze_rayleigh[t_graze_rayleigh < 0.0] = 0.0
        ...
        if self.enable_pinn:
            with torch.no_grad():
                ...
                alpha_shape = alpha_t2.shape
                inputs = self.normalize_inputs(torch.tensor(alpha_t2).to(self.dev).reshape(-1, 100), a, torch.tensor(R_pl).to(self.dev))
                x = torch.ones(inputs.shape[0]).unsqueeze(1).detach().to(self.dev)
                # y = torch.ones(inputs.shape[0]).unsqueeze(1).detach().to(self.dev)
                ys = torch.tensor(2 * ((radius - radius[-1]) / (radius[0]-radius[-1])) - 1).unsqueeze(1).to(self.dev)

                #inputs = torch.cat((x.repeat(100,1), y.repeat_interleave(x.shape[0]), inputs.repeat(100,1)), dim=-1).float()

                n_rep = torch.ceil(torch.tensor(x.shape[0]*100/self.batch_size)).int().item()
                ind = torch.ceil(torch.tensor(100/n_rep)).int().item()
                ...
                for i in range(n_rep):
                    if i != n_rep-1:
                        ...
                        ind = 100 - i*ind
                        y = ys[-ind:,:]
                    inputs_ = torch.cat((x.repeat(ind,1), y.repeat_interleave(x.shape[0], dim=0), inputs.repeat(ind,1)), dim=-1).float()
                    t_graze.append(self.model(inputs_).detach().cpu().numpy())
                    #t_graze[i] = torch.reshape(t_graze[i], alpha_shape[:-1]).unsqueeze(-1).detach().cpu().numpy()

                t_graze = np.concatenate(t_graze, axis=0).reshape((100,-1)).swapaxes(0,1).reshape(alpha_shape)

                alphas_t2 = inputs[:,2:].reshape(alpha_shape).detach().cpu().numpy()
                mask = alphas_t2 <= -1.5
                t_graze[mask] = 1.0

                t_graze[t_graze > 1.0] = 1.0
                t_graze[t_graze < 0.0] = 0.0
        ...
        if self.enable_rayleigh:
            t_graze[:,:,:1,:] = t_graze[:,:,:1,:] * t_graze_rayleigh
        
        self.alphas = alpha_t2
        self.transmission = t_graze.copy()
        ...
        self.final_transm = t_graze.copy()
